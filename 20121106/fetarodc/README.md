...
db.eval()を無効にしてサーバサイドでのjavascriptを許可しない。
```
> db.eval("print('test')")
...
16Mだと12000のネームスペースに有効。MAXは2G。
それぞれのコレクションは一つのネームスペースとしてカウントされる。インデックスも同じ。
一つのコレクションが一つのインデックスを持っている場合、6000コレクションまで持つことができる。

多くのコレクションを作る必要がある場合、nssizeを大きくしてください。
--nssize は新規に作られる .ns ファイルに対してだけ適応されることに注意してください。
既存のデータベースをリサイズしたい場合には、 --nssize をつけてデータベースを起動した後で、 
db.repairDatabase() コマンドをシェルから起動し、サイズを調整してください。

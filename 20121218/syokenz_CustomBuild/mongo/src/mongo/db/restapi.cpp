    ...
    public:
        ...
        virtual void handle( const char *rq, string url, BSONObj params,
            ...
            //fujisaki add start
                ...
                responseCode = 200;
                handleDelete( fullns , MiniWebServer::body( rq ) , params , responseCode , ss  );
            }
            //fujisaki add end
            else {
                responseCode = 400;
        ...
        // fujisaki add start
        void handleDelete( string ns, const char *body, BSONObj& params, int & responseCode, stringstream & out ) {
            try {
                BSONObj obj = fromjson( body );
                db.remove( ns.c_str(), obj, 1 );
            }
            catch ( ... ) {
                responseCode = 400; // Bad Request.  Seems reasonable for now.
                out << "{ \"ok\" : false }";
